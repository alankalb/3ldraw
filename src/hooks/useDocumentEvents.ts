...
export function useDocumentEvents() {
  ...
  const isAppFocused = useValue('isFocused', () => editor.getIsFocused(), [
    editor,
  ]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'Tab': {
          if (isFocusingInput() || editor.getOpenMenus().length) {
            return;
          }
          break;
        }
    ...
    function handleTouchStart(e: TouchEvent) {
      if (container.contains(e.target as Node)) {
        ...
        if (
          ...
          touchXPosition + touchXRadius >
            editor.getViewportScreenBounds().width - 10
        ) {
